...
- `resolve({ session?, mode? })`, which returns a complete per-call `SandboxExecutionPolicy`: a process-wide `enabled: false` setting > explicit approved mode > the session fold > `defaultMode`, and the session's immutable cwd > configured `workspaceRoot` fallback.
