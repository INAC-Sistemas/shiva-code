...
| `tsconfig.host.json` | The former root aggregate, moved verbatim: host packages, examples, tests, scripts; excludes `packages/client` | Yes |
...
The solution root `extends` the base deliberately: `examples/` and `scripts/` have no nearer tsconfig, so tsx (get-tsconfig) resolves their workspace imports through the root file. `extends` restores the `paths` map there while `files: []` keeps the file program-less. Their *type checking* is unaffected by this: examples and scripts files are included by the host aggregate.
