...
逐级进入文件夹、直接编辑路径、用前缀过滤最后一栏，或用头部下方的搜索框收窄屏幕上的分栏；宿主标记的隐藏条目默认不显示，直到页脚开关揭开。**新建文件夹**打开一个嵌套创建对话框，目标为选中的文件夹，并选中它创建出来的那个；**打开**采纳选中的文件夹，没有选中时回落到当前层级。确认一个目录即为选中的路径；关闭对话框即为取消。
...
- **搜索只覆盖已列出的分栏，且无多选、无重命名或删除** —— 对话框只负责列出与创建目录；搜索框收窄的是屏幕上已有的层级，别处的目标仍靠导航或编辑路径抵达。递归搜索需要 Host 侧的遍历，而 `host.listDirectory` 原语并不提供。
