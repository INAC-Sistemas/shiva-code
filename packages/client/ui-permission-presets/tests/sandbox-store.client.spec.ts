...
/** The sandbox controller over a real mirror and one scripted context. */
function sandboxController(api: object) {
  const ctx = { remote: { settings: api } } as never
  const mirror = new SettingsDescribeMirror(ctx)
  return { mirror, controller: new SandboxSettingsController(mirror, ctx) }
}
...
describe('sandbox settings store', () => {
  ...
  it('hides the row in a remote browser instead of loading forever', async () => {
    ...
    const controller = new SandboxSettingsController(mirror, ctx)
    await controller.load()
    expect(controller.store.getSnapshot().status).toBe('unavailable')
    await controller.setEnabled(false)
