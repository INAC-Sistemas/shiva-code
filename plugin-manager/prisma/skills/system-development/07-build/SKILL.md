...
3. **Spawn qa-tester**: write the tests for the ticket's "Done when" (typecheck, unit, regression, flow) under `testes/`. It writes them and stops — running the battery is `/08-review`'s question to the requester, not this loop's gate.
...
5. **Spawn evaluator**: "does the diff match the ticket's requirements AND the epic artifacts?" GREEN → set `status: human_test` (queued for the owner's end-of-epic batch — not an invitation to test now). RED → mismatch list goes back to the builder.
...
7. Next ticket, or the next phase once its predecessors are `human_test`. Two consecutive rounds with the **same** finding = stall: stop and escalate with `ask_user_question`.
