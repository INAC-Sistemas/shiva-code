...
**UI tickets** add one line to the role: "Load the `shadcn-ui` and `ui-icons` skills first; build every standard control from shadcn/ui components added with its CLI, using the template, base and preset from `04-tech-plan.md`, and take every icon from the pack that same plan records." The evaluator's briefing adds the matching checks: a hand-written control that a shadcn component covers is RED, so is a hand-written SVG or emoji that Lucide or Tabler covers, and so is a UI change without a real-browser screenshot.
