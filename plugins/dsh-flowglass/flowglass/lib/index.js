...
const makeStaticRegistry = () => {
  ...
  return {
    ...
    async panel(root, call) {
      ...
      if (!entry) return { ok: false, error: 'Tool not registered: ' + (toolId || '(empty)') }
      try {
...
// ===== shared-host.js：Inject到每个 Host-only 工具包开头的公共辅助（make-payloads.mjs Auto拼接）=====
// HTML 转义（面板内容来自 Host 拼接，转义User数据防止破坏结构）
...
// 幂等注册到Toolbox框架：框架未启动时每 500ms 快重试；注册成功后降为 2000ms 慢心跳
// （注册表实例更换——ToolboxPlugins重启/更新——时Auto重注册）；Plugins停止时Auto从注册表移除（Tab 级联消失）
const tryRegisterTool = (ctx, desc, handler) => {
  ...
  const ivFast = ctx.interval(() => {
    ...
    once()
    if (!had && off && !ivSlow) {
      // 刚注册成功：停快重试，改慢心跳（框架重启导致注册表实例更换时仍能Auto挂上）
      try { ivFast() } catch (e) {}
...
// ===== Session日志读取（带缓存；日志只追加 ⇒ count 不变即命中）=====
// 活Session读内存快照（零 IO）；持久化Session增量 readFrom，失败回退全量 readSession。
...
// ===== 仓库根发现（clone 部署关键）：Toolbox数据/产物一律归属「本仓库根」，而非Session cwd =====
// 场景：本仓库被 clone 到别的项目根目录下当子目录（如 D:\\other\\dsh-dynamic-toolbox\\），
// DSH 在宿主项目根运行——Session cwd / workspaceRoot 都是宿主项目，若按Session cwd 落盘会污染宿主。
// 这里先直下找 plugins.json，找不到再扫一级子目录（plugins.json 在子目录里即为本仓库）。
// 数据目录名由仓库根的 toolbox.config.json 配置（dataDir，Default .dsh-dynamic-toolbox）。
...
const findRepoRoot = async (ctx) => {
  ...
  // 必须解析出清单且含 id:'toolbox' items目——这是本仓库的强标记，杜绝误判锁定错误仓库根。
...
// 数据目录名：读仓库根 toolbox.config.json 的 dataDir（Default .dsh-dynamic-toolbox）
...
// 写策略：有Session按Session resolve（cwd 即可写边界）；无Session显式 workspace-write@仓库根 ——
// 绝不回落部署Default策略（其可写根是宿主进程 cwd，写工作区会被 FS_SANDBOX_DENIED 拒绝）。
...
const readJsonStore = async (ctx, rel, wsRoot, fallback) => {
  ...
  } catch (e) { return fallback } // resolve/stat IO 失败：不动原Files，按缺省处理
  let raw = null
  ...
  try {
    ...
    try {
      ...
      console.warn('readJsonStore: JSON parse failed, original content quarantined as backup (' + rel + ')')
    } catch (e2) {
      console.warn('readJsonStore: JSON parse failed and quarantine backup did not succeed (' + rel + ')')
    }
    return fallback
  }
...
// 优先按Session cwd，其次动作透传的 root，最后沙box工作区根；返回 { root, session }
...
// 注意：system 并入首条 user 消息Text（GenerateOptions 的 system 角色 source 契约未公开，此形态与 ask 一致、最稳）。
const AI_USAGE_REL = '.dsh-dynamic-toolbox/toolbox-ai-usage.json'
// ===== 用量台账写锁：per-root promise 链串行化所有对 toolbox-ai-usage.json 的读-改-写 =====
// 背景：chat 的 track 是响应后异步追加，compare 多模型并发时多个 RMW 同帧起跑会互相整Files覆盖丢记录；
...
const makeLlmHelper = (ctx) => {
  ...
  // provider 拓扑变化（适配器注册/注销）时清空缓存，避免模型清单陈旧（ctx.on 随Plugins停止Auto清理）
  ...
  // 路由解析：state 选择 → 当前SessionDefault → 第一个 provider 的第一个模型；
  ...
  const chat = async (st, system, user, timeoutMs, track) => {
    if (!llm) return { err: 'llm service unavailable' }
    if (!st.provider || !st.model) return { err: 'No model route selected' }
  ...
  const routeRow = (st, route, note) =>
    '<div class="tb-row">' +
      '<select class=”tb-select” data-field=”provider” data-action-onchange=”route” title=”Provider (model list auto-refreshes after switching)”>' +
...
// ===== 内容产物目录约定：<仓库根>/<dataDir>/data/<Pluginskey>/ =====
// 与 <dataDir>（工具内部 JSON 状态）分家：这里放人会直接打开的内容产物（Jira 附件、导出件等）。
// 点号目录与仓库 .git 同族、不污染根目录观感；所有Plugins产物收一处，.gitignore 只需一行 <dataDir>/data/。
...
// 把Plugins注册的模型工具归「Plugins」——沙box内 ctx.tools.get 被刻意降级为 schema 视图，清单是事实源）。
...
// 裸 await handle.done 遇到 git 凭证 GUI 弹窗/网络盘锁Files会永久挂起。withDeadline 在 ms 到点时
...
const withDeadline = (ctx, handle, ms) => {
  ...
  } catch (e) { /* timer service unavailable：退化为无看门狗（与旧行为一致） */ }
...
const callNativeBridge = async (name, request) => {
  ...
  if (!handler) return { ok: false, error: 'Native RPC not registered: ' + name }
...
const harness = {
  ...
  registerTool() { throw new Error('Model tool services are not enabled in the current static collection') },
}
...
export async function apply(ctx) {
  ...
  console.log(TOOLBOX_RUNTIME.logTag() + ' native static Host loaded (features: ' + registry.tools().map((x) => x.id).join(', ') + '）')
}
