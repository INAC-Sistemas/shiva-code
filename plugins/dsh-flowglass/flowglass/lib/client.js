...
// 动态模式：只拼接本Files（无 TOOLBOX_RUNTIME_OVERRIDES）→ 全部动态Default值，与历史行为一致。
  // 原生静态模式：构建器在本Files之前拼接 `const TOOLBOX_RUNTIME_OVERRIDES = {...}` JSON 字面量。
// 配置不走 globalThis/window/process.env（多 bundle 同进程会互相覆盖、批准包必须可审计、
// 全局变量会让 payload 内容哈希不能代表真实行为）——业务实现只读本Files定义的 TOOLBOX_RUNTIME。
const TOOLBOX_RUNTIME = (() => {
  ...
  return Object.freeze({
    ...
    // DOM 标记值：动态Default保持历史值（mounted="1"、entry=""），编译模式用 bundleId 区分多 bundle
...
// ===== toolbox-client.js：Toolbox框架 Client 半 — Drawer + Tab 栏 + 通用 HTML 面板壳 =====
// 工具列表经 host.call(RT.rpc('tools')) 轮询（1.5s，Drawer打开时）；
...
return {
  ...
  apply(ctx) {
    // ===== 命名空间（TOOLBOX_RUNTIME 由拼接的 shared/runtime.js 提供；动态Default=历史名称）=====
    const RT = TOOLBOX_RUNTIME
    // 原生 Flowglass bundle Inject Harness 官方 Markdown renderer；其他静态合集与
    // 动态Development模式没有这两个词法绑定时保持纯Text详情，不改变 Toolbox 通用契约。
    ...
    // 进程级单例（bundle 级）：页面已有本 bundle 主实例的Drawer时，本Session Client 半空转——避免重复注册
    // sidebar.footer.action / shell.overlay 造成双Drawer。marker 值为空格分隔的 bundleId 列表
    // （多 bundle 同进程各挂各的Drawer，互不抢占）；标记随 fiber 卸载只移除自己那一份。
    ...
    if (typeof document !== 'undefined' && document.body) {
      ...
      if (cur.indexOf(myDomId) >= 0) {
        console.log('toolbox: this toolbox drawer already exists on the page (main instance of another session); this session’s Client half skips it')
        return
      }
    ...
    // Flowglass 的“子代理跟随”要走 Harness 正式Session导航，不直接改 localStorage。
    // 子代理优先使用 catalog 的精确 address；若 catalog 尚未拉取，先刷新父Session再解析。
    const navigateHarnessSession = async (request) => {
      if (!request || !request.sessionId) return
      if (!sessionsClient) throw new Error('sessions service unavailable')
    ...
    // app-shell 每次切Session都会写 localStorage['dsh.sessions.current']（浏览器全局权威）。
    // Drawer主实例挂在宿主Session下，useSessions 不响应 UI 切Session——本桥 patch Storage.setItem，
    // 写入该 key 的瞬间 dispatch 自定义事件，Drawer监听后立即读取（零延迟），替代轮询主路径。
    ...
    // 覆盖——不与 React Text节点打架。running 计数直接统计面板 DOM 里「可见且 running」的行：
    // 官方 badge 统计整个进程的动态Plugins，toolbox/plugins 只有当前Toolbox的单仓库清单，
    // 用它计算会偏小；面板未打开时行不渲染，官方Text本就是正确的全局值，不覆盖。
    // 开关与管理视图共用、localStorage 持久化，Default开。动态模式保留历史 key；编译 bundle 按 bundleId 隔离。
    ...
    // 静态合集（capabilities.managePlugins=false）没有Plugins inventory——plugins RPC 恒返回空清单，
    // 联动启用也只会把 headless 清空、一行都藏不住：整体不启用（不装 CSS/observer/轮询、不渲染开关）。
    const panelHideSupported = RT.capabilities.managePlugins !== false
    // styles.insert 返回 disposer——必须挂 ctx.effect，否则Plugins停止/Rerun后旧样式残留，
    // Rerun一次叠一份（PLUGIN-DEV.md 规则4；ThemePlugins同款约束）
    ...
    const toolboxCss = [
      ...
      // ---- 侧边栏导航条目（DOM Inject，样式对齐任务看板/SSH 导航行；无 DOM 环境走 Slot 兜底） ----
      ...
      // better-sidebar 嵌入态：只复用面板，不带 fixed Drawer几何、阴影、拖拽 chrome。
      ...
      // 挤压三栏停靠：Drawer fixed 右栏（宽度内联给），主内容列 margin-right 让位（见 Drawer 内 effect），左中右并列互不遮挡
      ...
      // ---- Drawer内滚动条统一接管：轨道透明（融入面板背景），thumb 用边框色；Webkit + Firefox 双写 ----
      ...
      // ---- 管理页树（分类节点 → Plugins节点；分类头可折叠、可接拖放） ----
      ...
      // ---- Picture-in-picture（Document PiP）：主Drawer DOM 的实时镜像窗口（脱离 WebUI 框体） ----
      ...
      // 镜像容器：Drawer克隆充满窗口；拖拽把手/吸附指示/尺寸徽章在 pip 里无意义隐藏
      ...
      // ===== 共享设计System（tb-）：工具面板 HTML 直接使用；颜色只消费 --tb-* 变量（带兜底），ThemePlugins在 :root 声明即可覆盖 =====
      ...
      // ---- 输入（控件高度统一走 token：--tb-ctl-h 标准高 / --tb-ctl-h-sm 紧凑高，Theme可覆盖） ----
      ...
      // .tb-pane-body 用 column-reverse —— DOM 最新在前 ⇒ 视觉越往下越新，且滚动条Default停在底部
      ...
      // 正常方向变体（Default column-reverse 是给轨迹的「最新在底、滚动条Default底部」；其他分栏工具用 tb-pane-col）
      ...
      '.tb-pill-warn{color:var(--tb-warn-text,#d4b95c);border-color:var(--tb-warn-border,rgba(212,167,44,.4))}', // 语义同 other（警示黄）；轨迹「Commands」类等需要语义名的场景用
      ...
      // 可点 pill（管理视图「重启后」Default启停）：幽灵虚线弱化存在感，悬停/开启才上 accent，与展示型 pill 拉开层级
      ...
      // ---- Files树（chevron 旋转 + Files夹/Files图标 + 展开态高亮） ----
      ...
      // 重试/失败徽标（FlowglassAssistant卡头）：等待=琥珀 / 成功=绿 / 失败与错误码=红 / 未成行=灰
      ...
      // 流程图画布：bg-base 实色 + blueprint 网格线（中性灰蓝极低透明，明暗两Theme均隐约可见；实色底防透明皮肤重影）
      ...
      // ---- 三列泳道（手绘参考图 2：左=子代理分支区 / 中=主干User·Assistant / 右=工具调用卡） ----
      ...
      // 并行调用组外框：同一步骤 >1 items调用时虚线框圈成一组，右上「并行 ×N」角标（底色盖边框）
      ...
      // 主线User/Assistant卡：新出现时固定流光一圈；Assistant仍在进行时，固定圈结束后由 fl-live 接续持续流光。
      '.fl-main-min-anim{position:relative;border-color:var(--tb-accent-border,rgba(91,141,239,.65))!important}',
      // Running卡片：渐变流光贴边转圈（conic-gradient 环形遮罩 + 角度动画）。
      ...
      // Assistant卡进行中同款流光 + 脉冲点（与工具卡视觉一致）
      ...
      // 卡片点击展开的完整详情（入=完整传入 JSON / 出=完整返回Text）
      ...
      // ---- 调用详情右侧浮层（不插入流程流撑高内容——展开/收起零跳跃，Close回原来位置）----
      ...
      // Default显示原始 pre，User点预览图标后才隐藏，避免详情打开时Text闪现。
    ...
    // disposer 挂 ctx.effect：Plugins停止/Rerun不回收会每次叠加一份 ~30KB 样式表（同 :100 处理）
    ...
    // 服务可晚于Flowglass出现/被 HMR 替换，所以用 ctx.inject 驱动，并用这个小 store
    // 让已挂载的独立入口/Drawer 同步让位。完整 dynamic-toolbox 不接管为“Flowglass” Tab。
    ...
    function Entry(props) {
      ...
      return React.createElement(
        'button',
        {
          ...
          title: RT.displayName + '(toolset)',
          onClick: () => store.toggle(),
        },
        props.wide ? RT.displayName : 'box',
      )
    }

    // ===== 侧边栏导航入口（DOM Inject） =====
    // 侧边栏导航区（新Session / 任务看板 / SSH 那一列）rc.7 仍无官方 Slot（footer 的
    // sidebar.footer.action 位置太差，User决策回导航区），任务看板与 dsh-ssh 同样走
    // DOM Inject + MutationObserver 自愈。条目插在「新Session」行之后的Plugins族块
    // （taskboard/ssh/toolbox）末尾——即 SSH 之后、工作区之前；族块相对定位保证多个
    // 自愈Plugins重插后顺序稳定。纯 DOM（不进 React 树），不会干扰宿主 reconciliation。
    ...
    // 侧边栏入口图标引用（mountSidebarEntry Inject后绑定；无 DOM 环境走 Slot 兜底无图标，保持 null）。
    // Drawer 在只剩一个工具时经 replaceSidebarIcon 把Toolbox图标临时换成该工具的图标。
    ...
    // ---- Drawer几何与激活 Tab 本地记忆（localStorage；无 localStorage 的环境静默跳过）----
    ...
    // ---- 工具分类（导航第二行 + 管理树共用）：Default表 + User覆盖（localStorage 持久化；键 = 工具/清单条目 id） ----
    const TOOL_CATS = [
      { id: 'ai', label: 'AI' },
    ...
    // Drawer导航临时记忆（客户端重载即清）：上次离开的 分类+工具；null = 重载后还没打开过Drawer
    let lastNav = null
    const catsRead = () => {
      try {
        if (typeof localStorage === 'undefined') return null
    ...
    // ===== 外观配置（Theme预设 / 自定义主色 / 界面与Detail font size）：localStorage 单一事实源 =====
    // 取代旧 theme-teal/theme-amber Plugins：同一机制（向 scope 根或 :root Inject --tb-* 变量），
    // 但配置收进管理页「外观」分区与 better-sidebar Flowglass设置页，两处共用本 store，改动即时生效。
    const APPEARANCE_PRESETS = { default: '', teal: '#0d9488', amber: '#d97706' }
    // 预设变量族沿用旧ThemePlugins原值（Teal-green/Warm orange观感不变）；自定义主色由 accentFamilyVars 现算
    ...
    // 详情侧拉框宽度：0 = Default（350px，CSS 兜底）；有效范围 240–1200px。
    ...
    // 自定义主色 → accent/active 全家族（亮色提亮用向白混合近似旧Theme的明暗关系）
    ...
    // 外观样式Inject：变更即重插；disposer 挂 ctx.effect（Plugins停止/Rerun不残留，同 :128 样式规则）
    ...
    // Drawer管理页与 better-sidebar Flowglass设置页共用本组件
    function AppearancePanel() {
      ...
      const sliderRow = (label, key, min, max) => React.createElement('div', { style: rowStyle },
        ...
        React.createElement('input', {
          ...
          title: label + 'Zoom (100% = built-in default)',
        ...
        Math.abs(a[key] - 1) > 0.001
          ? React.createElement('button', {
            ...
            title: 'Restore 100%', onClick: () => appearanceWrite({ [key]: 1 }),
          }, '↺')
          : null,
      )
      return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '8px' } },
        React.createElement('div', { style: rowStyle },
          ...
          Object.keys(APPEARANCE_PRESETS).map((k) => {
            ...
            return React.createElement('button', {
              key: k, type: 'button',
              title: k === 'default' ? 'Built-in default blue' : k === 'teal' ? 'Teal-green (original theme plugin color)' : 'Warm orange (original theme plugin color)',
            ...
            }, (k === 'default' ? 'Default' : k === 'teal' ? 'Teal-green' : 'Warm orange') + (APPEARANCE_PRESETS[k] ? '' : ''))
          }),
          React.createElement('input', {
            type: 'color', value: a.accent || APPEARANCE_PRESETS[a.preset] || '#3f6fd9',
            title: 'Custom accent color (overrides the theme presets)',
          ...
          a.accent
            ? React.createElement('button', {
              ...
              title: 'Clear the custom accent color and return to the theme preset', onClick: () => appearanceWrite({ accent: '' }),
            }, '✕ Custom')
            : null,
        ),
        sliderRow('UI font size', 'uiScale', APPEARANCE_SCALE_MIN, APPEARANCE_SCALE_MAX),
        sliderRow('Detail font size', 'detailScale', APPEARANCE_SCALE_MIN, APPEARANCE_SCALE_MAX),
        // 详情侧拉框宽度：0 = Default 350px；拖 rail 左缘手柄与滑杆写同一配置
        React.createElement('div', { style: rowStyle },
          React.createElement('span', { style: lblStyle }, 'Detail width'),
          React.createElement('input', {
            type: 'range', min: APPEARANCE_RAIL_MIN, max: APPEARANCE_RAIL_MAX, step: 10, value: String(a.railW || 350),
            title: 'Detail drawer width (also adjustable by dragging the drawer’s left edge; remembered automatically)',
            style: { flex: '1', minWidth: '110px', accentColor: 'var(--tb-accent,#3f6fd9)' },
        ...
        // 预览块：内联按倍率现算 px（不依赖 CSS 变量）——管理页看不到Flowglass本体、sidebar 设置弹层
        // 又在本 bundle scope 之外，两处都靠它即时看效果；侧边模式下面板背后的Flowglass本身也在实时变
        React.createElement('div', {
          ...
          React.createElement('span', { style: { fontSize: '10px', opacity: 0.6, letterSpacing: '.4px' } }, 'Preview (changes live as you drag the slider)'),
          React.createElement('div', { style: { fontSize: (12.5 * a.uiScale).toFixed(1) + 'px', lineHeight: 1.5 } },
            'UI text Aa123 — buttons / labels / message preview'),
          React.createElement('div', {
            style: {
          ...
          }, 'In · full input {”seq”: 12}\nOut · full return text detail sample Aa123'),
        ),
      )
    }

    // better-sidebar Flowglass Tab 设置页：Display mode（原 pluginToggles 声明改为自绘 select）+ 外观面板
    function BetterSidebarFlowSettings(props) {
      ...
      // 显式按 DSH 明暗Theme给 select/option 配色，colorScheme 让原生弹层跟随
      ...
      return React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '10px', fontSize: '12.5px' } },
        React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '5px' } },
          React.createElement('span', { style: headStyle }, 'Display mode'),
          React.createElement('select', {
            ...
            React.createElement('option', { value: 'auto', style: optStyle }, 'Auto — side card when available'),
            React.createElement('option', { value: 'sidebar', style: optStyle }, 'Side card — Flowglass tab powered by better-sidebar'),
            React.createElement('option', { value: 'drawer', style: optStyle }, 'Standalone drawer — keeps Flowglass’s original standalone entry point and drawer'),
          ),
        ),
        React.createElement('div', { style: { display: 'flex', flexDirection: 'column', gap: '6px' } },
          React.createElement('span', { style: headStyle }, 'Appearance (theme / accent color / font size)'),
    ...
    function Drawer(props) {
      ...
      const [autoMs, setAutoMs] = React.useState({}) // toolId -> Auto刷新毫秒（面板 HTML 声明 data-autorefresh 驱动）
      const [tabBadges, setTabBadges] = React.useState({}) // toolId -> Tab 角标Text（面板 HTML 声明 data-tab-badge 驱动；借鉴 better-sidebar tab 角标）
      ...
      const [tabFilter, setTabFilter] = React.useState('') // 工具搜索（整行长条；仅Session内存）
      ...
      // refreshTools 在DrawerClose的首帧 effect 中也可能立即拿到静态工具清单；必须在任何 early return 前初始化。
      ...
      // 明暗Theme：theme 服务持有偏好；ThemeSnapshot 无顶层 colorScheme，生效明暗在 snapshot.active.colorScheme
      const schemeOf = (snap) => {
      ...
      // ---- Picture-in-picture（Document PiP）：主Drawer DOM 的实时镜像（不是独立实现） ----
      // 原则：主Drawer React 树是唯一事实源；pip 窗口显示其 DOM 克隆（MutationObserver debounce 同步），
      // 交互事件按索引路径代理回主 DOM 触发——两种模式显示与行为完全一致；
      // 主Drawer全程照常运行（轮询/状态不受影响）；pip 未开启时零开销、零影响。
      // ⚠️ 整体禁用开关：所有情况下都不渲染Picture-in-picture按钮、不开启 PiP 窗口（当前为 false，
      // 已按需求把所有情况下的Picture-in-picture都关掉）；恢复时改回 true 即可，其余逻辑无需变动。
      ...
      const [mainHidden, setMainHidden] = React.useState(false) // pip 期间主Drawer可视觉隐藏（DOM 存活：镜像源与轮询不断）
      ...
      // 侧边栏重新点开Drawer → Cancel视觉隐藏；Drawer被真正Close → 联动关 pip（镜像源消失）
      React.useEffect(() => {
        if (isOpen && mainHidden) setMainHidden(false)
        if (!isOpen && pipRef.current) closePip()
      }, [isOpen])
      // 组件卸载/PluginsRerun → 必关 pip（防孤儿窗）
      React.useEffect(() => () => {
        const cur = pipRef.current
        pipRef.current = null
        if (!cur) return
      ...
      // 索引路径：镜像内元素 ↔ 主Drawer元素（cloneNode 同构，children 索引一一对应）
      ...
      const openPip = async () => {
        ...
        try {
          ...
          // 1. 克隆样式表（link 克隆 / style 标签深克隆；tb- 设计System与宿主 dsw- 变量一并带走）
          ...
          // 2. Theme同步（body[data-ds-dark-theme] + colorScheme；theme/change 跟随）
          const syncTheme = () => {
            try {
          ...
          // PiP 克隆保留 bundle root attribute（Theme变量/CSS scope 挂在 bundle root 上）
          ...
          barTitle.textContent = RT.displayName + ' · picture-in-picture (content identical to the main window; actions sync in real time)'
          const btnMax = win.document.createElement('button')
          btnMax.className = 'jr-pip-bar-btn'
          btnMax.textContent = '⤢ Enlarge'
          btnMax.title = 'Enlarge to 72%×88% of the screen (click again to restore)'
          const btnBack = win.document.createElement('button')
          btnBack.className = 'jr-pip-bar-btn'
          btnBack.textContent = '✕ Back to main window'
          btnBack.title = 'Close picture-in-picture and return to the main-window drawer'
          ...
          const resync = () => {
            ...
            // 脱离 fixed 外壳语义（充满 pip 窗口）；主Drawer被视觉隐藏时镜像保持可见
          ...
          // 7. Theme跟随 + pip 窗口被关（System X）→ 释放
          ...
          // 并存模型：不关主Drawer（它是镜像事实源，轮询照跑）；User可点主Drawer X 仅视觉隐藏（mainHidden）
        } catch (e) {}
      }
      ...
      const htmlScrollRef = React.useRef(null) // 静默刷新前记录的滚动位置（effect 里恢复，防Auto刷新打断阅读）
      const seqRef = React.useRef({}) // toolId -> 最新请求序号：丢弃过期响应，防 provider/模型联动竞态
      // 流程右侧调用卡的最短动画跨 innerHTML 轮询续播：scope 隔离主/子Session，key 对应调用 seq。
      ...
      const openRef = React.useRef(isOpen) // 独立Drawer或嵌入 Tab 的统一可见状态
      openRef.current = isOpen
      const retryCountRef = React.useRef({}) // toolId -> Panel failed to load重试次数（一次性重试，非轮询；成功清零）
      ...
      const lastPanelActiveRef = React.useRef(null) // 离开再返回 Flowglass 时重置为最近 60 items
      const flowScrollByScopeRef = React.useRef(new Map()) // 主/子Flowglass各自的阅读位置
      const flowScrollIntentRef = React.useRef(null) // enter=新子Flowglass到底；back=恢复上级
      ...
      const flowHarnessNavTargetRef = React.useRef(null) // 只有 Flowglass 主动导航的目标Session才恢复返回链
      ...
      const toggleFlowZen = async () => {
        ...
        catch (e) { setError('The browser refused native fullscreen; fell back to viewport Zen mode') }
      }
      ...
      // Drawer 直接订阅 DSH SessionRuntime.list，为工作区Session树提供完整、实时的 ids/byId。
      React.useEffect(() => {
        if (RT.bundleId !== 'flow') return undefined
        const list = sessionsClient && sessionsClient.list
      ...
      // 挤压三栏：full 模式给主内容列（DSH grid 的 centerCol）加 margin-right 让出Drawer宽度，
      // 聊天区收缩而非被覆盖（grid items margin 在轨道内生效，不影响侧边栏轨道）；
      // CloseDrawer/切模式还原，拖拽调宽实时跟随；React 不管该列内联 style，不会被覆盖
      React.useEffect(() => {
      ...
      React.useEffect(() => {
        ...
        // 明确声明Default到底部的正常方向面板（当前为「上下文」）优先于旧 Tab 的滚动位置恢复。
        ...
        // 切工具/新面板不恢复旧位置：正常方向（tb-pane-col）容器Default滚到最底查看
      ...
      // dangerouslySetInnerHTML 会整块替换卡片 DOM，因此用调用 key Save截止时间，并在每次替换后把类续挂到新节点。
      React.useEffect(() => {
        ...
        if (store.scope !== scope) {
          clearStore()
          store.scope = scope
          // 首次打开建立历史基线，但最新User卡不能被吞掉：它通常就是触发本轮流程的当前消息。
          // DOM 因 column-reverse 不是时间顺序，按 seq 数值找最新User卡。
      ...
      // 工具与流式Assistant的运行耗时不依赖 2s 面板轮询：Client 每 250ms 就地更新一次。
      React.useEffect(() => {
      ...
      React.useLayoutEffect(() => {
        ...
        // 新打开的Assistant详情Default预览；User对当前详情显式切回Text后，
        // Auto刷新保持Text，Close详情后清掉该例外，下次打开Restore defaults预览。
      ...
      // —— 当前Session/工作区解析（v6.5）——
      // Drawer主实例挂在宿主Session（toolbox-host-*）下，其 useSessions 不响应浏览器 UI 切Session；
      // app-shell 会把当前Session写入 localStorage['dsh.sessions.current']（浏览器全局权威）。
      // 来源：localStorage 事件桥（SESSION_EVENT，切Session零延迟，主路径）
      ...
      // 触发 SESSION_EVENT（见 apply layers patch），立即刷新——零延迟，替代轮询主路径。
      React.useEffect(() => {
      ...
      // Session上下文检测：记录「上次已处理过」的 cwd 与 session id（只在 effect 内更新，
      ...
      const branchFlowAt = async (seq) => {
        ...
        if (!sessionsClient || typeof sessionsClient.fork !== 'function') { setError('The current Harness version does not support session branching'); return }
        setFlowUiBusy(true); setFlowUiNotice('Creating branch session…')
        try {
          ...
          setFlowBringPopup(false)
          setFlowTreeOpen({})
        } catch (e) { setError('Branch failed: ' + String((e && e.message) || e)) }
        finally { setFlowUiBusy(false) }
      }

      const fetchSelectedFlowContext = async () => {
        if (!flowSelectedSeqs.length) throw new Error('Please box-select flow cards first')
        ...
        if (!res || !res.ok || !res.flowContext || !res.flowContext.text) throw new Error((res && res.error) || 'Could not read the selection')
        return res.flowContext
      }

      const putFlowContextIntoDraft = (sessionId, text, append) => {
        if (!sessionsClient || typeof sessionsClient.provideInfo !== 'function') throw new Error('The current Harness version does not support cross-session draft writes')
        ...
        if (!actions || typeof actions.setDraft !== 'function') throw new Error('The target session’s input area is unavailable')
        let next = text
      ...
      const createSelectedFlowSession = async () => {
        ...
        if (!sessionsClient || typeof sessionsClient.create !== 'function') { setError('The current Harness version does not support creating blank sessions'); return }
        setFlowUiBusy(true); setFlowUiNotice('Creating a new session from the selected content…')
        try {
        ...
        } catch (e) { setError('Failed to create session: ' + String((e && e.message) || e)) }
        finally { setFlowUiBusy(false) }
      }

      const sendSelectedFlow = async () => {
        const target = flowTargetSession
        if (!target) { setFlowUiNotice('Select a target session'); return }
        if (!sessionsClient) { setFlowUiNotice('Harness session service unavailable'); return }
        setFlowUiBusy(true); setFlowUiNotice('Organizing and bringing in the selected content…')
        try {
          const context = await fetchSelectedFlowContext()
          ...
          setFlowBringPopup(false)
          setFlowTreeOpen({})
          setFlowUiNotice('Appended to the target session’s draft')
        } catch (e) { setFlowUiNotice('Carry-in failed: ' + String((e && e.message) || e)) }
        finally { setFlowUiBusy(false) }
      }

      // Zoom 仅缩放Flowglass画布内容，header/工具栏与滚动容器保持原尺寸。
      React.useEffect(() => {
      ...
      React.useEffect(() => {
        ...
        const onUp = (e) => {
          ...
          if (d.moved) {
            ...
            // 空白处单击：Cancel框选并Close详情侧拉框（落在卡片或交互控件上则不干预，保留其原生行为）
            const t = e.target
      ...
      async function loadPanel(toolId, action, el, opts) {
        ...
        const silent = Boolean(opts && opts.silent) // 静默刷新（Auto轮询）：不转圈、不清错误
        ...
        try {
          ...
          // 5s 无响应即视为失败，走下方一次性重试，避免面板永久停在「Loading panel…」
          ...
          if (res && res.ok) {
            ...
            // Session scope 变化（Harness 自己切换Session）Default进入新Flowglass底部；
            // Flowglass 内部 fback 则由 flowScrollIntentRef 恢复上级Save位置。
            const currentFlow = panelRef.current && panelRef.current.querySelector('[data-flow][data-flow-scope]')
            const currentScope = currentFlow && (currentFlow.getAttribute('data-flow-scope') || '')
            ...
            // 任何动作都Save滚动位置（Auto轮询/展开详情/提交等）：全量 innerHTML 重渲染会丢滚动/选择，
            ...
            setHtml(res.html)
            // Host 只在 Flowglass 已开启“子代理跟随”且User主动点击进入时返回该指令。
            // 使用 SessionRuntime 导航会让 Harness 的Session列表、Session页和持久选中态一起更新。
            if (toolId === 'flow' && res.navigateSession) {
              ...
              if (target) {
                ...
                // 因此在 Client 独立暂存，等目标 Session 真正成为 current 后再Inject。
              ...
              try { await navigateHarnessSession(res.navigateSession) }
                ...
                setError('Flowglass switched, but Harness failed to follow: ' + String((e && e.message) || e))
              }
            }
            // Auto刷新约定：工具 HTML 带 data-autorefresh="ms" → Drawer静默定时重拉（静默 = 不转圈）
            ...
            // Tab 角标约定：工具 HTML 带 data-tab-badge="Text" → 该工具 Tab 显示角标（空字符串=清除）
            ...
            // copy 契约：Host 附带 copy chars串 → 写剪贴板并短暂提示
            if (typeof res.copy === 'string' && res.copy) {
              try {
                if (typeof navigator !== 'undefined' && navigator.clipboard && navigator.clipboard.writeText) {
                  await navigator.clipboard.writeText(res.copy)
                  setCopied('Copied ' + res.copy.length + ' chars')
                } else {
                  setCopied('No clipboard API in this environment')
                }
              } catch (e) {
            ...
            setError((res && res.error) || 'Panel failed to load')
            retryOnce(toolId)
          }
        } catch (e) {
          if (seqRef.current[toolId] === seq) {
            setError('Panel request error: ' + String((e && e.message) || e))
            retryOnce(toolId)
          }
        } finally {
      ...
      // 最长 2 runs、间隔 1.2s；仅当Drawer仍打开且 active 未变才重试。成功清零（见 loadPanel 成功分支）。
      ...
      // 关键修复：面板是 dangerouslySetInnerHTML Inject的裸 DOM，其中的 <select> 没有 React fiber；
      ...
      // 原生 change 事件正常冒泡——改在Drawer根节点挂原生监听（随DrawerClose卸载、重开重挂）。
      React.useEffect(() => {
      ...
      async function refreshTools() {
        ...
        // 否则Drawer打开前的挂载刷新会把 active 清掉并落盘，打开后出现无 tab 激活的假死态
        if (!list.length) return
        // 无有效记忆时的Default Tab：Session「流程」（User指定）；没有 flow 才退到列表第一个。
        // 分类芯片必须同步切到目标工具的分类（flow 在「Session」），否则面板与 Tab 栏错位
        const cur = activeRef.current
      ...
      // ===== Plugins生命周期开关（齿轮管理视图）：直连 Host 半 toolbox/plugins + toolbox/plugin-toggle =====
      ...
      async function togglePlugin(p) {
        ...
        try {
          ...
          if (r && !r.ok) setError(r.error || 'Plugin action failed')
          else if (r && r.warning) setRebuildLines(['⚠ ' + String(r.warning)]) // 启停已生效但Failed to write start/stop memory to disk：不静默
        } catch (e) {
          setError('Plugin action error: ' + String((e && e.message) || e))
        }
        refreshPlugins()
        refreshTools() // 停止 → 注册表级联移除 Tab；启动 → 500ms 重试注册后Auto回来
      }

      // Rerun类操作后的落定刷新：等注册表 500ms 重试周期过后再拉工具列表并重载当前面板——
      ...
      // Rerun单个Plugins：桩重读磁盘 impl，改完代码点这个即生效
      async function restartPlugin(p) {
        ...
        try {
          ...
          if (r && !r.ok) setError(r.error || 'Plugin rerun failed')
        } catch (e) {
          setError('Plugin rerun error: ' + String((e && e.message) || e))
        }
        refreshPlugins()
        settleAfterRestart()
      }

      // 批量启停：Host-only Plugins一次全停/全启，启停记忆统一落盘
      async function toggleAll(enable) {
        ...
        try {
          ...
          const lines = []
          if (!r) lines.push('Batch action got no response')
          else {
            if (r.error) lines.push('✗ ' + r.error)
            if (r.warning) lines.push('⚠ ' + String(r.warning))
            if (Array.isArray(r.done) && r.done.length) lines.push((enable ? 'Started: ' : 'Stopped: ') + r.done.join('、'))
            if (Array.isArray(r.skippedClient) && r.skippedClient.length) lines.push('Skipped (has a Client half; operate it in the Cordis panel): ' + r.skippedClient.join('、'))
            if (Array.isArray(r.failed) && r.failed.length) lines.push('Failed: ' + r.failed.join('；'))
            if (lines.length === 0) lines.push('No actionable plugins')
          }
          setRebuildLines(lines)
        } catch (e) {
          setRebuildLines(['Batch action error: ' + String((e && e.message) || e)])
        }
        refreshPlugins()
        refreshTools()
      }

      // 「重启后」pill 点击：只改启停记忆（下次重建的Default启停），不动当前运行态
      async function setDefaultPlugin(p) {
        ...
        try {
          ...
          if (r && !r.ok) setError(r.error || 'Failed to set default start/stop')
        } catch (e) {
          setError('Default start/stop setting error: ' + String((e && e.message) || e))
        }
        refreshPlugins()
      }

      // 批量Rerun：Running的 Host-only Plugins全部重启（桩重读磁盘 impl）——改完多个工具一键生效
      async function restartAll() {
        setError(null)
        try {
          const r = await host.call(RT.rpc('plugin-restart-all'), { root: currentCwd || undefined, session: currentSessionId || undefined })
          const lines = []
          if (!r) lines.push('Batch rerun got no response')
          else {
            if (r.error) lines.push('✗ ' + r.error)
            if (Array.isArray(r.done) && r.done.length) lines.push('Reran: ' + r.done.join('、') + '(major panel bodies such as preview/diff have been reset along with their closures)')
            if (Array.isArray(r.skippedClient) && r.skippedClient.length) lines.push('Skipped (has a Client half; operate it in the Cordis panel): ' + r.skippedClient.join('、'))
            if (Array.isArray(r.failed) && r.failed.length) lines.push('Failed: ' + r.failed.join('；'))
          ...
          setRebuildLines(['Batch rerun error: ' + String((e && e.message) || e)])
        }
        refreshPlugins()
        settleAfterRestart()
      }
      ...
      async function runRebuild() {
        ...
        try {
          ...
          const lines = []
          if (!r) lines.push('Rebuild request got no response')
          else {
            if (r.error) lines.push('✗ ' + r.error)
            if (Array.isArray(r.defined) && r.defined.length) lines.push('New definitions: ' + r.defined.join('、'))
            if (Array.isArray(r.started) && r.started.length) lines.push('Started: ' + r.started.join('、'))
            if (Array.isArray(r.skipped) && r.skipped.length) lines.push('Skipped (already defined / framework’s own): ' + r.skipped.join('、'))
            if (Array.isArray(r.suppressed) && r.suppressed.length) lines.push('Kept off (start/stop memory): ' + r.suppressed.join('、'))
            if (Array.isArray(r.failed) && r.failed.length) lines.push('Failed: ' + r.failed.join('；'))
            if (typeof r.ms === 'number') lines.push('Elapsed: ' + r.ms + 'ms (payload disk read + define + run, serial)')
            if (lines.length === 0) lines.push('All plugins in plugins.json already exist; nothing to rebuild')
          }
          setRebuildLines(lines)
        } catch (e) {
          setRebuildLines(['Rebuild error: ' + String((e && e.message) || e)])
        } finally {
          setRebuilding(false)
        }
        loadRebuildHistory()
      ...
      React.useEffect(() => {
        ...
        // Drawer打开时的导航恢复（User指定）：重载后「第一次」打开 → Default Session+流程；之后打开 →
        ...
        const offOpen = store.subscribe(() => {
          ...
          // 有记忆恢复记忆（工具已停则由 refreshTools 兜底回Default并同步分类）；无记忆Default Session+流程
      ...
      // Session/工作区切换跟随：当前Session的 cwd 或 session id 任一变化 = 切Session或切工作区。
      // 两个仓库工具 id 同名时 refreshTools 的「active 保持」分支不会重载面板，且 loadPanel
      // 只在 active 变化时触发——切Session（同工作区或跨工作区）active 不变，面板仍显示旧Session
      ...
      React.useEffect(() => {
        ...
        // 清Session级缓存（cwd 或 sid 任一变化）：工具的 st.sid 是Session级 state（flow/trace 等
        // 优先 st.sid 渲染），切Session不清理会沿用旧Session id——必须清空让工具从新Session重算。
        ...
        if (cwdChanged || sidChanged) {
          ...
          if (sidChanged) {
            ...
            if (!isFlowFollow) {
              // User手动切换 Session：不沿用之前 Flowglass 的临时返回链。
        ...
        if (cwdChanged) {
          // 同时清上一工作区的在屏 HTML 与逐工具请求标记：经过无Toolbox的工作区后延迟重载
      ...
      // 单工具图标替换：只剩一个非Toolbox工具时，侧边栏入口的「Toolbox」图标换成该工具的图标；
      // 多工具 / 管理视图恢复Toolbox图标。（图标随注册表下发，此项只消费 tools 的 icon 字段，平日不显示）
      React.useEffect(() => {
      ...
      // 面板Auto刷新：当前工具的 HTML 声明了 data-autorefresh="ms" 时，静默轮询（不转圈、不抢滚动——silent 路径）
      ...
      // isOpen 守卫：DrawerClose时不预加载面板（页面加载早期 RPC 通道未就绪，
      // 提前 loadPanel 会永久卡住 → 首次打开显示「Loading panel…」；打开Drawer时
      // isOpen 变化重新触发本 effect 正常加载）
      React.useEffect(() => {
        if (!isOpen) return
        if (!active) { lastPanelActiveRef.current = null; setHtml(null); return }
        // localStorage 记忆可能指向Stopped的工具：tools 已加载且不含它时等 refreshTools 纠正，不闪错误
        ...
        if (active === 'flow' && switched) {
          // 分页只是本次阅读状态：切换 Tab 后回来从最近 60 items重新开始。
      ...
      // 无 tab 激活 + 面板永久「Loading panel…」，手动切 tab 才好）。这里按 tools 强制不变量：
      // Drawer开着且列表非空时，active 无效 → 兜底 flow/第一个（交给激活 effect 接管加载）；
      ...
      // 详情内容框Copy：读同段 .fl-pre 的 textContent（浏览器已把实体解码回原文）写剪贴板，
      // 纯客户端不绕 Host；1.2s 后经 ctx.timeout 复位按钮文案（Plugins停止随生命周期清理）
      ...
      // 派发走Drawer根节点的原生 change 监听（见 loadPanelRef 旁的修复注释），不要用 React onChange 属性。
      ...
      const flowZoomControl = active !== 'flow' || managing || flowBringPopup ? null : React.createElement('div', { className: 'tb-flow-zoom-float' + (flowSelectedSeqs.length ? ' with-selection' : '') },
        React.createElement('div', { className: 'tb-flow-zoom', title: 'Zooms the Flowglass canvas without changing header and scroll area sizes' },
          React.createElement('button', { type: 'button', className: 'tb-flow-zoom-btn', disabled: flowZoom <= 60, onClick: () => stepFlowZoom(-1) }, '−'),
          React.createElement('button', { type: 'button', className: 'tb-flow-zoom-value tb-flow-zoom-btn', title: 'Restore 100%', onClick: () => setFlowZoomLevel(100) }, flowZoom + '%'),
          React.createElement('button', { type: 'button', className: 'tb-flow-zoom-btn', disabled: flowZoom >= 150, onClick: () => stepFlowZoom(1) }, '+'),
          React.createElement('button', {
            type: 'button', className: 'tb-flow-zoom-btn', title: flowZen ? 'Exit Zen native fullscreen (Esc)' : 'Enter Zen native fullscreen', onClick: toggleFlowZen,
      ...
      const flowSelectionBar = active !== 'flow' || managing || !flowSelectedSeqs.length ? null : React.createElement('div', { className: 'tb-flow-selection-bar' },
        React.createElement('span', { className: 'tb-flow-selection-count', title: 'Selected ' + flowSelectedSeqs.length + ' items' }, flowSelectedSeqs.length),
        React.createElement('button', {
          type: 'button', className: 'tb-flow-icon-btn', disabled: flowUiBusy,
          title: 'New session from selection: writes only a draft of the selected content; inherits no other history', onClick: createSelectedFlowSession,
        ...
        React.createElement('button', {
          type: 'button', className: 'tb-flow-icon-btn', disabled: flowUiBusy,
          title: 'Carry into session', onClick: () => { setFlowBringPopup(true); setFlowTreeOpen({}); setFlowUiNotice('') },
        }, React.createElement('svg', { viewBox: '0 0 16 16', width: 14, height: 14, fill: 'none', stroke: 'currentColor', strokeWidth: 1.3, strokeLinecap: 'round', strokeLinejoin: 'round', 'aria-hidden': true },
        ...
        React.createElement('button', {
          type: 'button', className: 'tb-flow-icon-btn', disabled: flowUiBusy,
          title: 'Clear box selection', onClick: () => { setFlowSelectedSeqs([]); setFlowBringPopup(false) },
      ...
      const flowBringDialog = active !== 'flow' || managing || !flowSelectedSeqs.length || !flowBringPopup ? null : React.createElement('div', { className: 'tb-flow-bring-popup' },
        React.createElement('div', { className: 'tb-flow-popup-head' },
          React.createElement('span', null, 'Carry into session · ' + flowSelectedSeqs.length + ' items'),
          React.createElement('button', { type: 'button', className: 'tb-flow-icon-btn', title: 'Close', onClick: () => setFlowBringPopup(false) }, '×'),
        ),
        ...
        React.createElement('div', { className: 'tb-flow-popup-actions' },
          ...
          React.createElement('button', { type: 'button', className: 'tb-flow-select-btn', disabled: flowUiBusy || !flowTargetSession, title: 'Appends to the target session’s draft; not sent automatically', onClick: sendSelectedFlow }, 'Carry into draft'),
        ),
      )

      // Theme切换按钮：暗色下显示太阳（点击切亮），亮色下显示月亮（点击切暗）；theme 服务缺失时不渲染
      const themeButton = !themeSvc ? null : React.createElement('button', {
        type: 'button',
        className: 'jr-overlay-close',
        title: themeScheme === 'dark' ? 'Switch to light theme' : 'Switch to dark theme',
      ...
      // Picture-in-picture按钮：浏览器支持 Document PiP 才渲染（Chrome/Edge 116+，localhost 安全上下文）
      const pipButton = !pipSupported ? null : React.createElement('button', {
        type: 'button',
        className: 'jr-overlay-close',
        title: 'Picture-in-picture: puts the toolbox in its own mini window (leaves the main window, draggable anywhere)',
      ...
      const gearButton = RT.capabilities.managePlugins === false ? null : React.createElement('button', {
        type: 'button',
        className: 'jr-overlay-close',
        title: managing ? 'Back to tool panel' : 'Manage plugins (stop/start)',
      ...
      const dockButton = React.createElement('button', {
        type: 'button',
        className: 'jr-overlay-close',
        title: dockMode === 'right' ? 'Switch to three-column docking (squeezes the chat area; left/center/right side by side)' : dockMode === 'full' ? 'Switch to floating' : 'Dock to the right',
        onPointerDown: (ev) => ev.stopPropagation(),
      ...
      if (!embedded && dockMode === 'full' && !width) style.width = '560px' // 三栏停靠Default宽
      // pip Picture-in-picture期间主Drawer可视觉隐藏（DOM 与轮询存活——它是镜像事实源）
      ...
      if (managing) {
            ...
            React.createElement('div', { className: 'tb-pane-head' },
              ...
              React.createElement('div', { className: 'tb-row' },
                React.createElement('button', {
                  ...
                  disabled: rebuilding,
                  title: canRebuildFromDisk ? undefined : 'Compile collection: restore define/run from the frozen capability manifest (no disk reads)',
                  onClick: runRebuild,
                }, rebuilding ? 'Rebuilding…' : (canRebuildFromDisk ? 'Rebuild/fill in from plugins.json' : 'Restore compiled manifest')),
                React.createElement('button', {
                  type: 'button',
                  className: 'tb-btn',
                ...
                React.createElement('button', {
                  type: 'button',
                  className: 'tb-btn',
                  title: 'Rerun all running Host-only plugins (one click brings edited tool code into effect for several tools at once; stopped ones stay put)',
                  onClick: restartAll,
                }, 'Rerun all'),
                React.createElement('button', {
                  type: 'button',
                  className: 'tb-btn tb-btn-ghost',
                  title: 'Hide all headless (Host-only) tools and show only plugins with a UI; click again to restore',
                  onClick: () => toggleAll(false),
                }, 'Stop all'),
                panelHideSupported ? React.createElement('button', {
                  type: 'button',
                  className: 'tb-chip' + (hideHostOnly ? ' tb-chip-on' : ''),
                  title: 'Hides the headless (Host-only) plugins in the Cordis panel at the bottom left (the manage-page list is unaffected); lit means currently hiding, click again to restore' + (hideHostOnly ? '; currently hiding ' + panelHide.headless.size + ' items' : ''),
              ...
              React.createElement('div', { className: 'tb-note', style: { marginTop: '2px' } },
                React.createElement('div', { style: { fontWeight: 600, marginBottom: '6px' } }, 'Appearance (theme / accent color / font size; takes effect immediately and is remembered)'),
                React.createElement(AppearancePanel, null)),
              rebuildLines
              ...
              rebuildHistory.length && canRebuildFromDisk
                ? (() => {
                    ...
                    return React.createElement('div', { className: 'tb-note' },
                      'Rebuild time (last ' + rebuildHistory.length + ' runs; hover for details; red = failures present)',
                      React.createElement('div', { style: { display: 'flex', alignItems: 'flex-end', gap: '3px', height: '36px', marginTop: '6px' } },
                        rebuildHistory.map((h, i) => React.createElement('div', {
                          key: i,
                          title: (h.at || '') + ' · ' + (h.ms == null ? '—' : h.ms + 'ms') + ' · defined ' + h.defined + ' / started ' + h.started + ' / stopped ' + h.suppressed + ' / failed ' + h.failed,
                          style: {
                            width: '10px',
              ...
              aiUsage && canAiUsage && aiUsage.totals && aiUsage.totals.calls
                ? React.createElement('div', { className: 'tb-note' },
                    'AI usage (last 100 ledger entries): total ' + aiUsage.totals.calls + ' runs · output ' + aiUsage.totals.out + ' tok' + (aiUsage.totals.errors ? ' · failed ' + aiUsage.totals.errors + ' runs' : '') + (aiUsage.totals.todayCalls ? '; today ' + aiUsage.totals.todayCalls + ' runs / ' + aiUsage.totals.todayOut + ' tok' : ''),
                    React.createElement('div', { className: 'tb-pills', style: { marginTop: '4px' } },
                      aiUsage.tools.map((t) => React.createElement('span', {
                        ...
                        title: t.tool + ': success ' + t.calls + ' runs · output ' + t.out + ' tok' + (t.errors ? ' · failed ' + t.errors + ' runs' : ''),
                      }, t.tool + ' ' + t.calls + '/' + (t.out >= 10000 ? (t.out / 1000).toFixed(1) + 'k' : t.out) + ' tok')),
                      React.createElement('button', {
                        type: 'button',
                        className: 'tb-btn tb-btn-sm tb-btn-ghost',
                        title: 'Copy ledger summary CSV (tool,calls,output_tokens,errors)',
                        onClick: async () => {
                          ...
                          try {
                            ...
                            } else setCopied('No clipboard API in this environment')
                          } catch (e) { setCopied('Copy failed: ' + String((e && e.message) || e)) }
                        },
                      }, 'Copy CSV')))
                : null,
              React.createElement('div', { className: 'tb-note' }, canRebuildFromDisk
                ? 'The toggle truly stops / truly starts the plugin (same as the Cordis panel’s stop/run; state stays in sync in both places) and also writes the start/stop memory .dsh-dynamic-toolbox/toolbox-plugins.json; the ”after restart” pill is the default start/stop for the next rebuild — click to toggle; it only changes the memory, not the current state. After stopping, Tabs cascade away; after starting, they reattach automatically in ~0.5s. Plugins with a Client half (framework): operate them from the Cordis panel.'
                : 'The toggle truly stops / truly starts the plugin (same as the Cordis panel’s stop/run; state stays in sync in both places) and also writes this collection’s start/stop memory; the ”after restart” pill is the default start/stop for the next launch — click to toggle; it only changes the memory, not the current state. After stopping, Tabs cascade away; after starting, they reattach automatically in ~0.5s. Plugins with a Client half: operate them from the Cordis panel.'),
            ),
            React.createElement('div', { className: 'tb-pane-body tb-pane-col' },
              (() => {
                ...
                if (visiblePlugins.length === 0) {
                  return React.createElement('div', { className: 'tb-notice' }, 'No dynamic plugins yet')
                }
                // 单个Plugins节点（行 key 用 pluginId，跨分类移动时稳定；条目 id 追加在副行便于辨认归属键）
                const manageRow = (p) =>
                      React.createElement('div', {
                        key: p.pluginId,
                        ...
                        p.entryId ? React.createElement('span', { className: 'tb-drag', title: 'Drag onto a category heading to change membership' }, '⋮⋮') : null,
                        ...
                        React.createElement('span', { className: 'tb-pill ' + (p.running ? 'tb-pill-done' : 'tb-pill-todo') }, p.running ? 'Running' : 'Stopped'),
                        p.defaultStart != null
                          ? React.createElement('button', {
                            type: 'button',
                            className: 'tb-pill-btn' + (p.defaultStart ? ' on' : ''),
                            title: 'Default start/stop for next rebuild (start/stop memory) — click to toggle; changes the memory only, leaving the current run state untouched',
                            onClick: () => setDefaultPlugin(p),
                          }, p.defaultStart ? 'Start after restart' : 'Stop after restart')
                          : null,
                        React.createElement('button', {
                          type: 'button',
                          className: 'tb-btn tb-btn-sm tb-btn-ghost',
                          title: p.hasClientHalf ? 'Has a Client half; operate it from the Cordis panel' : 'Rerun (reloads the implementation from disk; click this after editing code)',
                        ...
                        React.createElement('button', {
                          ...
                          // canStop=false：宿主缺少 stopFromPanel（停止降级）——Running的Plugins开关禁用，停止的仍可启动
                          title: p.hasClientHalf ? 'Has a Client half; operate it from the Cordis panel'
                            : p.running ? (p.canStop === false ? 'Current DSH lacks the stop API (stopFromPanel); cannot stop' : 'Stop this plugin')
                            : 'Start this plugin',
                    ...
                    for (const p of visiblePlugins) {
                      const cid = p.entryId ? catOf(p.entryId) : 'system' // 清单外Plugins归「System」且不可拖（无稳定归属键）
                      ...
                      .map((c) => {
                        ...
                        return React.createElement('div', { key: 'cat-' + c.id },
                          React.createElement('div', {
                            ...
                            title: 'Click to collapse/expand; drop a node onto this heading to change its category (stays in sync with the navigation row)',
                            ...
                            React.createElement('span', { className: 'tb-note' }, list.length + ' items · ' + running + ' running'),
                          ),
        ...
        body = currentCwd
          ? React.createElement('div', { className: 'tb-empty' },
              canRebuildFromDisk
                ? 'No dsh-dynamic-toolbox detected in the current workspace\nLoads automatically once you switch to a workspace containing a toolbox; tools are not shown in this workspace'
                : 'This toolbox collection is not mounted in the current workspace\nLoads automatically once you switch to a valid workspace; tools are not shown in this workspace',
            )
          : React.createElement('div', { className: 'tb-empty' },
              'No tools yet\nTools appear here automatically once a tool plugin (e.g. Jira) runs; stopped plugins can be restarted from the manage button at the top right',
            )
      } else {
        body = React.createElement('div', { className: 'tb-frame', ref: panelRef, onClick: onPanelClick, onKeyDown: onPanelKeyDown, onPointerDown: onFlowRailResizeDown },
          ...
          showJumpLatest ? React.createElement('button', {
            ...
            title: 'Smoothly scroll back to the panel’s latest content',
      ...
      const drawerEl = React.createElement('div', {
        ...
        // bundle root：CSS scope / Theme变量挂载点（多 bundle 共存隔离；PiP 克隆随 DOM 属性一并镜像）
        ...
        embedded ? null : React.createElement('div', {
          ...
          React.createElement('span', { className: 'jr-drawer-title' }, managing ? RT.displayName + ' · manage' : RT.displayName),
          themeButton,
          pipButton,
          gearButton,
          dockButton,
        ...
        // 只有一个工具时整条导航无意义：隐藏搜索工具栏/分类行/Tab 栏，Drawer退化为纯面板；
        ...
        managing || tools.length === 1 ? null : React.createElement('div', { className: 'tb-nav' },
          React.createElement('input', {
            ...
            placeholder: 'Search tools (total ' + tools.length + ' items, matched by name / ID)',
            value: tabFilter,
          ...
          React.createElement(HRow, { className: 'tb-hrow tb-cats' },
            visibleCats.map((c) => React.createElement('button', {
              ...
              title: 'Category ”' + c.label + '” — the manage page (manage button at top right) lets you drag nodes into another category',
              onClick: () => pickCat(c.id),
            }, c.label + ' · ' + catCounts[c.id]))),
          React.createElement(HRow, { className: 'tb-hrow tb-tools' },
            shownTools.length
              ? shownTools.map(toolButton)
              : React.createElement('span', { className: 'tb-hrow-empty' }, searching ? 'No matching tools' : 'No running tools in this category')),
        ),
    ...
    function FlowglassSidebarView(props) {
      ...
      if (!active) {
        ...
        }, React.createElement('div', { className: 'tb-notice' }, 'Flowglass is currently set to standalone drawer mode.'))
      }
      return React.createElement(Drawer, {
        ...
        // 嵌入态由 better-sidebar scope 提供权威Session，不再读取 shell overlay 的 hook。
        useSessions: () => undefined,
      })
    }

    if (typeof document !== 'undefined' && typeof MutationObserver !== 'undefined') {
      // 侧边栏导航区无官方 Slot：DOM Inject导航条目（新Session下方、SSH 之后），disposer 随Plugins停止清理
    ...
    // better-sidebar 自身也通过同一个服务注册Built-in Tab；Flowglass只消费公开契约，
    // 不 value-import 对方代码。服务缺失时 inject fiber 保持等待，独立Drawer照常工作。
      ...
      ctx.inject(['betterSidebar'], (sidebarCtx) => {
        ...
        const descriptor = {
          ...
          settings: features.indexOf('pluginSettings') < 0 ? undefined : {
            // 自绘设置面板：Display mode（持久化走 pluginSettings.displayMode）+ 外观（Theme/主色/字号，
            // 读写Flowglass自身 localStorage store，与管理页「外观」分区同源）
    ...
    async function apply(ctx) {
      ...
      if (!remote) throw new Error('Static toolbox Remote namespace not mounted: ' + "toolboxNativeFlow")
      const methods = {
      ...
      host = {
        call(methodName, args) {
          ...
          if (!method) return Promise.resolve({ ok: false, error: 'Native static collection does not support manage actions: ' + methodName })
