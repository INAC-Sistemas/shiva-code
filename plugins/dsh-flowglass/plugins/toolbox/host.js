// ===== toolbox-host.js：Toolbox框架 Host 半 — 工具注册表 + 面板 RPC + Plugins生命周期开关 =====
// 工具Plugins（Host-only）通过 ctx.get(TOOLBOX_RUNTIME.registryService).register(...) 注册；
...
return {
  ...
  async apply(ctx) {
    ...
    // 编译模式：静态 Bootstrap 提供的 namespaced Service——内嵌清单/payload，不扫仓库、不读源码仓库Files。
    // 本Files主体只依赖统一接口，不再直接假设 plugins.json 必然存在。
    const makeDynamicDiskProvider = () => {
      ...
      return {
        ...
        // 启停记忆写：走 subprocess（与Auto补齐报告同路径，绕过 fs 沙box策略）
    ...
    // 状态挂在服务对象上（跨 fiber 共享）：root → 工具表；build 锁式 runInBuild 见该Files注释。
    ...
    // 框架驱动的 runner.run 必须在注册表互斥 build 段内执行（工具Plugins apply 里 register 归入
    // 本 root 的表）——手动启停/Rerun/批量/重挂/重建都走这里，避免落 lastRoot 单槽归错仓库。
    const runInBuild = (root, fn) => registry.runInBuild(root, fn)

    // 本框架所属仓库根：探测所有候选（sandboxPolicy.workspaceRoot + 各Session cwd，含一级子目录），
    ...
    // agent 解析：live agent 优先；宿主Session/幽灵 id 等非 live Session兜底为最小 agent
    // （runner 的内部调用只消费 agent.id；steer/inject 在 agents.get 查不到时本就静默）。
    // 自举宿主Session模式下主实例归属「宿主Session id」，此兜底让Drawer的管理 RPC 也能驱动它。
    const agentFor = (sid) => sid ? ((agents && agents.get(sid)) || { id: sid }) : undefined

    // 宿主Session id（与 host-bootstrap/index.js hostIdOf 同算法，两处必须同步）：唯一标识一个仓库根，
    // 用于Auto补齐时把「本框架」与其宿主Session行对上（多仓库并存不误认别的框架行）。
    ...
    // owner 校验：bootstrap 行的 owner 是本仓库宿主Session（hostIdOf(root) 精确匹配）；手动定义
    // 的旧行（owner 为真实Session）按 owner Session cwd 是否包含本仓库 root 判定；owner 无从查证
    // → 不属于。清单读失败（空映射）时退回按调用方Session过滤（原降级路径）。
    ...
    // owner Session的 cwd（手动定义行的归属判定用；宿主垫片不在 sessions 服务里，返回 undefined）
    ...
    // owner 归属：owner Session工作区包含本仓库 root（findRepo 只在 cwd 直下与一级子目录探测，
    ...
    // 当前仓库的动态Plugins清单（inventory 是全进程的，按「清单归属」过滤为当前仓库的行；
    // 自举宿主Session模式下Plugins可能挂在宿主Session名下，不再按 agentId===session 过滤）
    // 附带 defaultStart：该Plugins在「下次重建」时的Default启停（启停记忆 .dsh-dynamic-toolbox/toolbox-plugins.json
    ...
    ctx.effect(() => harness.handle(RT.rpc('plugins'), async (args) => {
      ...
      for (const r of runner.inventory()) {
        ...
        rows.push({
          pluginId: r.pluginId,
          name,
          entryId: meta ? meta.entryId : null, // 清单条目 id（== 工具 id）；清单外Plugins为 null（管理树归「System」且不可移动）
          running: Boolean(r.activeRun),
          currentPackageId: r.currentPackageId || null,
          // 含 Client 半的Plugins启停涉及浏览器编排/批准，交给 Cordis 面板
    ...
    // 行内可运行的 Package：current/next 指针优先；被抑制Plugins（重建时只 define 未 run，
    ...
    // 真停/真启：stop 走 stopFromPanel（与面板一致，会向SessionInject通知）；
    ...
    ctx.effect(() => harness.handle(RT.rpc('plugin-toggle'), async (args) => {
      ...
      if (row.packages.some((p) => p.hasClientHalf)) {
        return { ok: false, error: pluginId + ' has a Client half; start/stop it from the Cordis panel' }
      }
      // runner 的 owned() 按「定义时的 sessionId」校验所有权：自举模式下Plugins挂在
      // toolbox-host-* 垫片Session下，用调用方Session的 agent 会被拒（"lost on DSH restart"）——
      // 一律按行归属Session取 agent（宿主垫片在 agents 服务里，非 live Session兜底最小 {id}）。
      ...
      if (enable) {
        ...
        return { ok: false, error: (res && (res.message || res.reason)) || 'Start failed' }
      }
      if (!canStopFromPanel) {
        return { ok: false, error: 'The current DSH lacks the runner.stopFromPanel API; cannot stop the plugin (starting is unaffected)' }
      }
      ...
      return { ok: false, error: (res && (res.message || res.reason)) || 'Stop failed' }
    }))
    ...
    // 无记录的条目按清单的 autoStart Default行为。
    // 动态模式落 <root>/<dataDir>/toolbox-plugins.json（subprocess 写，绕过 fs 沙box）；
    // 编译模式由静态 Bootstrap 的 provider 持久化（不落User工作区，见 DSH_TOOLBOX_COMPILED_BUNDLES_PLAN §10.3）。
    ...
    const writeConfig = async (root, cfg) => {
      ...
      // 否则「重启后」pill 与重建Default值冻结在框架启动后首次读取）
    ...
    // 动态 pluginId → 清单条目 id：按当前 Package 名匹配指定仓库 plugins.json items目名
    ...
    // warning 字段（开关本身已生效，持久化结果不许静默：Plugins.md §7 红线）。
    const persistToggle = (pluginId, enabled, root) => withConfigLock(root, async () => {
      try {
        ...
        return ok ? null : 'Failed to write start/stop memory to disk (this toggle took effect, but the ”after restart” default may not match the UI)'
      } catch (e) { return 'Error writing start/stop memory: ' + String((e && e.message) || e) }
    })

    // 只改「下次重建Default启停」（启停记忆），不动当前运行态——管理视图「重启后」pill 的点击链路
    ctx.effect(() => harness.handle(RT.rpc('plugin-set-default'), async (args) => {
      ...
      if (!pluginId) return { ok: false, error: 'Missing pluginId' }
      const hit = await manifestEntryIdOf(pluginId, root)
      if (!hit) return { ok: false, error: 'Plugin is not in the plugins.json manifest; cannot write start/stop memory' }
      ...
      if (!written) return { ok: false, error: 'Failed to write start/stop memory to disk' }
      return { ok: true, entryId: hit.entryId, enabled: Boolean(args && args.enabled) }
    }))

    // Rerun单个Plugins：桩在 apply 时重读磁盘 impl——改完 plugins/<key>/tool.js 点它即生效，
    // 不用重新 define/批准。等同 toggle(enable=true) 但允许对Running的Plugins执行（真重启）。
    ctx.effect(() => harness.handle(RT.rpc('plugin-restart'), async (args) => {
      ...
      if (!row || !isRepoRow(row, byName, root, sid)) return { ok: false, error: 'Plugin does not exist or does not belong to the current repository: ' + pluginId }
      if (row.packages.some((p) => p.hasClientHalf)) {
        return { ok: false, error: pluginId + ' has a Client half; rerun it from the Cordis panel' }
      }
      const pkg = pkgOf(row)
      if (!pkg) return { ok: false, error: pluginId + ' has no runnable Package' }
      ...
      return { ok: false, error: (res && (res.message || res.reason)) || 'Rerun failed' }
    }))

    // 批量启停：一次动作完成当前仓库全部 Host-only Plugins的真停/真启，启停记忆统一写一次
    ctx.effect(() => harness.handle(RT.rpc('plugin-toggle-all'), async (args) => {
      ...
      if (!enable && !canStopFromPanel) {
        return { ok: false, error: 'The current DSH lacks the runner.stopFromPanel API; cannot stop in batch (starting is unaffected)' }
      }
      ...
      const out = await withConfigLock(root, async () => {
        ...
        for (const r of runner.inventory()) {
          ...
          // 按行归属Session取 agent（自举Plugins挂宿主垫片Session，调用方Session agent 过不了 owned()）
          const rowAgent = agentFor(r.agentId)
          if (enable) {
            if (r.activeRun) { done.push(r.pluginId + '(already running)') }
            else {
              const pkg = pkgOf(r)
              if (!pkg) { failed.push(r.pluginId + ': no runnable Package'); continue }
              const res = await runInBuild(root, () => runner.run(rowAgent, r.pluginId, pkg, 'run'))
              if (res && res.ok) done.push(r.pluginId)
              else { failed.push(r.pluginId + ': ' + ((res && (res.message || res.reason)) || 'Start failed')); continue }
            }
          } else {
            if (!r.activeRun) { done.push(r.pluginId + '(was already stopped)') }
            else {
              const res = await runner.stopFromPanel(rowAgent, r.pluginId)
              if (res && res.ok) done.push(r.pluginId)
              else { failed.push(r.pluginId + ': ' + ((res && (res.message || res.reason)) || 'Stop failed')); continue }
            }
          }
        ...
        if (cfg && cfgRoot) {
          ...
          if (!okWrite) warning = 'Failed to write start/stop memory to disk (this batch of toggles took effect, but the ”after restart” default may not match the UI)'
        }
    ...
    // 批量Rerun：对当前Running的 Host-only Plugins逐个 run（桩重读磁盘 impl）——
    // 改完多个 tool.js / shared/host.js / 磁盘加载器后一键全部生效，不用逐行点「Rerun」。
    // 停着的Plugins不动（尊重开关状态，不隐式启动）；含 Client 半的跳过（去 Cordis 面板）。
    ctx.effect(() => harness.handle(RT.rpc('plugin-restart-all'), async (args) => {
      ...
      for (const r of runner.inventory()) {
        if (!isRepoRow(r, byName, root, sid)) continue
        if (r.packages.some((p) => p.hasClientHalf)) { skippedClient.push(r.pluginId); continue }
        if (!r.activeRun) continue // 只RerunRunning的
        const pkg = pkgOf(r)
        if (!pkg) { failed.push(r.pluginId + ': no runnable Package'); continue }
        const res = await runInBuild(root, () => runner.run(agentFor(r.agentId), r.pluginId, pkg, 'run'))
        if (res && res.ok) done.push(r.pluginId)
        else failed.push(r.pluginId + ': ' + ((res && (res.message || res.reason)) || 'Rerun failed'))
      }
      return { ok: failed.length === 0, done, failed, skippedClient }
    }))

    // ===== 从清单自举重建（齿轮按钮 + 启动Auto补齐共用 doRebuild）=====
    // 动态模式框架读磁盘 payload.json（本身就是 define 参数的完整 JSON），编译模式读 provider
    // 内嵌 payload，经 dynamicCordisRunner 批量 define + run Host-only Plugins——全新重建缩到
    // 「define+run 框架 + 一次批准」，零点击。
    // 幂等：按Plugins name 跳过本仓库已定义的（含被User停掉的，尊重开关状态）。
    // 启停记忆：读 provider 启停配置，记录为Close的条目只 define 不 run（恢复上次记录）。
    // v6.3：串行 + build 上下文——工具Plugins在 apply 里注册必须归入本仓库 root，串行保证
    ...
    const doRebuild = async (sid, root) => {
      ...
      const found = await findManifest(root || undefined)
      if (!found || !found.manifest || !Array.isArray(found.manifest.plugins)) {
        return { ok: false, error: (artifacts.mode === 'compiled-bundle' ? 'Compiled manifest unavailable (root not attached)' : 'Cannot find plugins.json') + (root ? '（root: ' + root + '）' : '') }
      }
      const manifest = found.manifest
      const manifestRoot = found.root
      const config = await readConfig(manifestRoot)
      const cfgPlugins = (config && config.plugins) || {}
      // 幂等按「本仓库宿主/本仓库Session」的行判定（评审 H4 修复）：isRepoRow 只按清单名匹配，
      ...
      const suppressed = []
      const failed = []
      const approvalPending = [] // approval items目：run 非阻塞发起 → 批准卡弹出，User点一次即启动（授权不跨进程，这是浏览器代码执行的安全闸门）
      ...
      // 会先后持锁重复 define 同一批Plugins——锁内现读 inventory，后到者看到先到者刚定义的行即跳过。
      await registry.runInBuild(manifestRoot, async () => {
        // 幂等快照在锁内采集（审计 M11），且按「仓库归属」判定而非仅当前Session（评审 P1 补充）：
        // 同仓库两个Session并发重建时，先到者 define 的行挂在它Session名下——只认当前 sid/hostOfRoot
        // 会让后到者视而不见、整批重复定义。owner 判定复用 sessionCwdOf/ownsRoot（与 isRepoRow
        // 同语义）：行归属Session的工作区包含本仓库 root 即算已定义。
    ...
    // AI 用量台账聚合（管理视图总行）：读 .dsh-dynamic-toolbox/toolbox-ai-usage.json，按工具聚合 runs数/输出token/失败
    ctx.effect(() => harness.handle(RT.rpc('ai-usage'), async () => {
      ...
      try {
        ...
        // 台账路径与写方（shared/host.js mapDataRel）同源：跟随 RT.dataDir，不再硬编码Default目录名
    ...
    // ===== 启动Auto补齐：动态模式框架每次启动自调一次 doRebuild（幂等，已定义的按名跳过）=====
    ...
    // 兜底：inventory 里按 plugins.json 的 toolbox items目 name 找框架自身所在行——
    // 恰好一行才采用（多Session同名框架并存时无法区分归属，宁可跳过也不补齐到别的Session）。
    let stopped = false
    ctx.effect(() => () => { stopped = true })
    if (artifacts.capabilities.rebuildFromDisk !== false) ;(async () => {
      // 分阶段落盘报告（subprocess 直写，绕过 fs 沙box策略）：每到一个阶段整份重写，
      // Files停在哪一阶段，问题就在哪一阶段之后。报告路径 <工作区>/.dsh-dynamic-toolbox/toolbox-autorebuild.json
      const stages = []
      const report = async (stage, extra) => {
        ...
        // 历史耗时曲线：读旧Files的 history 追加本次 done 结果（最近 20 runs），重建速度变化可追踪
        let history = []
        try {
      ...
      if (!sid) {
        try {
          // 兜底：优先本框架仓库的宿主Session（agentId === hostIdOf(myRoot)）匹配框架行
          ...
          if (sName) {
            ...
            else if (anyHits.length > 1) console.log('toolbox: auto-fill skipped (same-named frameworks coexist across multiple repositories and the host cannot tell them apart); rerun from drawer management if needed')
          }
        } catch (e) {}
      }
      await report('sid', { sid: sid || null })
      if (stopped) { await report('stopped-before-rebuild'); return }
      if (!sid) { console.log('toolbox: auto-fill skipped (cannot determine the current session)'); return }
      const res = await doRebuild(sid, myRoot)
      if (stopped) { await report('stopped-after-rebuild'); return }
      // 确定性重挂：框架重启后注册表是全新空表，Running的PluginsRerun一遍，
      // 让注册确定性落进新表（2s 慢心跳在服务重 provide 后因子 fiber 的 ctx.get 命中
      // isolate key 变化而 throw、无法自愈，必须Rerun重建 fiber；刚由 doRebuild 启动的跳过避免双跑）
      ...
      const reattachAsync = [] // 含 Client 半的Plugins：Rerun为异步 starting，浏览器端另行激活
      // 排除框架自身（清单 id=toolbox 对应Plugins）：否则 reattach Rerun框架 → 框架重启再 reattach → 无限重启循环
      ...
      if (!reattachEnabled) {
        console.log('toolbox: remount skipped (' + (!res || !res.ok ? 'doRebuild did not succeed' : !selfName ? 'Manifest has no toolbox entry' : selfPluginIds.size === 0 ? 'Could not identify the framework itself' : 'sid not determined') + '), running tools rely on heartbeat/manual recovery')
      }
      if (reattachEnabled) {
        ...
        for (const r of runner.inventory()) {
          ...
          try {
            ...
            if (rr && rr.ok) {
              ...
              reattachFailed.push(r.pluginId + ': ' + ((rr && (rr.message || rr.reason)) || 'Rerun failed'))
            }
          } catch (e) {
      ...
      if (reattached.length) console.log('toolbox: framework restarted; deterministically remounting running tools: ' + reattached.join('、'))
      if (reattachAsync.length) console.log('toolbox: remounting plugins with a UI (async activation): ' + reattachAsync.join('、'))
      if (reattachFailed.length) console.log('toolbox: remount failed: ' + reattachFailed.join('；'))
      await report('done', { sid, res, reattached: reattached.length + reattachAsync.length })
      if (res && res.ok) {
        if (res.defined && res.defined.length) {
          console.log('toolbox: auto-fill new definition: ' + res.defined.join('、') + '; started: ' + (res.started || []).join('、'))
        } else {
          console.log('toolbox: auto-fill check complete; all plugins in plugins.json already exist')
        }
        if (res.failed && res.failed.length) console.log('toolbox: auto-fill partially failed: ' + res.failed.join('；'))
        if (res.suppressed && res.suppressed.length) console.log('toolbox: staying off as recorded last time: ' + res.suppressed.join('、'))
        if (res.approvalPending && res.approvalPending.length) console.log('toolbox: waiting for approval to start (approval card is already shown; one Allow click starts it): ' + res.approvalPending.join('、'))
      } else {
        console.log('toolbox: auto-fill failed: ' + ((res && res.error) || '(unknown)'))
      }
    })().catch((e) => { console.log('toolbox: auto-fill error: ' + String((e && e.message) || e)) })

    // Session信息查询（v6.5）：client 按当前Session id 查 header.cwd。
    // Drawer主实例挂在宿主Session下，useSessions 视角的 byId 记录可能不可靠——
    // cwd 以 Host 侧 sessions / sessionQuery 为准（client 每次切Session后调用）。
