window.__ModuleLoader__.load({ id: 'dsh-docs-panel', factory: (require) => {
  ...
  // React 通过模块加载器Inject提供，不是浏览器全局变量
  const React = require('react')

  // 客户端 → 宿主端 RPC：经同源 HTTP 路由（正式Plugins形态）
  ...
  function apply(ctx) {
    ...
    // betterSidebar 是跨Plugins服务（由 dsh-better-sidebar 提供），不能放进静态
    // inject——否则在 better-sidebar 尚未就绪的冷启动阶段，本Plugins会一直 pending
    ...
    injectStyles([
      ...
      // 视觉对齐 better-sidebar Built-in面板：header 36px、icon 按钮 28px 圆形、
    ...
    // ---------- Global docs组件（better-sidebar tab 内容） ----------
    ...
    function DocsView() {
      ...
      React.useEffect(function () {
        ...
        rpc('docs.list').then(function (res) {
          ...
          if (res && res.ok) {
            ...
            setDocs([])
            setListError((res && res.error) || 'Failed to read document list')
          }
        }).catch(function (err) {
          if (!alive) return
          setListLoading(false)
      ...
      function openDoc(rel) {
        ...
        rpc('docs.read', { rel: rel }).then(function (res) {
          ...
          if (res && res.ok) {
            ...
            setContent(null)
            setReadError((res && res.error) || 'Read failed')
          }
      ...
      function saveConfig(forceDir) {
        ...
        if (dir.length === 0) {
          setConfigMsg({ kind: 'err', text: 'Path cannot be empty' })
          return
        }
        setConfigMsg(null)
        rpc('config.set', { docsDir: dir }).then(function (res) {
          if (res && res.ok) {
            setConfigMsg({ kind: 'ok', text: 'Saved: ' + dir })
            setConfigDir(res.docsDir || dir)
            setEditDir(res.docsDir || dir)
            refreshList()
          } else {
            setConfigMsg({ kind: 'err', text: (res && res.error) || 'Save failed' })
          }
        }).catch(function (err) {
      ...
      function openExternal(app) {
        ...
        rpc('docs.open', { rel: selected, app: app }).then(function (res) {
          ...
          setOpenMsg((res && res.error) || 'Open failed')
        }).catch(function (err) {
      ...
      const tocPanel = selected !== null && tocOpen ? React.createElement('div', { className: 'dsp-toc' },
        tocItems.length === 0
          ? React.createElement('p', { className: 'dsp-note' }, 'This document has no title')
      ...
      const toolbar = selected !== null && readError === null ? React.createElement('div', { className: 'dsp-toolbar' },
        React.createElement('div', { className: 'dsp-toolbar-row' },
          React.createElement('button', {
            ...
            title: 'Open this document in Chrome',
            onClick: function () { openExternal('chrome') },
          }, '🌐 Open in Chrome'),
          React.createElement('button', {
            type: 'button',
            className: 'dsp-toolbar-btn',
            title: 'Open this document in VS Code',
          ...
          React.createElement('button', {
            ...
            title: 'Expand or collapse the outline',
      ...
      return React.createElement('div', { className: 'dsp-root' },
        React.createElement('header', { className: 'dsp-header' },
          React.createElement('span', { className: 'dsp-title' }, 'Global docs'),
        ...
        settingsOpen ? React.createElement('div', { className: 'dsp-settings' },
          React.createElement('span', { className: 'dsp-settings-label' }, 'Documents directory'),
          ...
          React.createElement('button', { type: 'button', className: 'dsp-btn', onClick: function () { saveConfig(undefined) } }, 'Save'),
          React.createElement('button', { type: 'button', className: 'dsp-btn', onClick: function () { saveConfig('~/.dsh/docs') } }, 'Restore defaults'),
          configMsg ? React.createElement('span', { className: 'dsp-msg ' + (configMsg.kind === 'ok' ? 'dsp-msg-ok' : 'dsp-msg-err') }, configMsg.text) : null,
          React.createElement('div', { className: 'dsp-settings-hint' }, 'Current directory: ' + configDir + '(saved in ~/.dsh/storages/dsh-docs-panel/config.json)')
        ) : null,
        toolbar,
        React.createElement('div', { className: 'dsp-body' },
          React.createElement('aside', { className: 'dsp-list' },
            listLoading ? React.createElement('p', { className: 'dsp-note' }, 'Reading document list…') : null,
            ...
            !listLoading && !listError && listItems.length === 0
              ? React.createElement('p', { className: 'dsp-note' }, 'No Markdown documents found. Put .md files into ' + configDir + ' to read them here.')
              : null,
            listItems
          ),
          React.createElement('main', { className: 'dsp-reader', ref: setReaderEl },
            selected === null ? React.createElement('p', { className: 'dsp-note' }, 'Pick a document from the list on the left to start reading') : null,
            contentLoading ? React.createElement('p', { className: 'dsp-note' }, 'Opening document…') : null,
