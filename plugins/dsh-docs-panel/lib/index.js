 ...
 * dsh-docs-panel 宿主端。
 *
 * 功能：Documents directory配置（存 ~/.dsh/storages/dsh-docs-panel/config.json，绝不用 settings.yaml，
 * 避免触发 settings 服务的 watcher 造成前端同步失效）、Markdown 文档扫描与读取、
 * Chrome/VS Code 外部打开、代码Copy到剪贴板。
...
// 若用 ctx.get() 同步读取，Plugins可能在服务尚未注册时提前 apply，
...
export function apply(ctx) {
  ...
  console.log('[dsh-docs-panel] loaded')

  // inject 已声明硬依赖，此处仅作防御性兜底
  if (fs === undefined || shell === undefined) return

  let homeCache = null
  // Documents directory配置的内存缓存（惰性加载/持久化，见 load/persistConfiguredDir）
  let docsDirCache = null

  // Documents directory配置：存 ~/.dsh/storages/dsh-docs-panel/config.json（自管 JSON，
  // 与 settings.yaml 彻底解耦）。未配置时用Default ~/.dsh/docs。
  // 结构同 cost-meter 的 storages/<plugin>/ 惯例，避免污染配置根目录。

  // 惰性读配置Files，填充缓存（首次调用或缓存为空时触发）
  async function loadConfiguredDir() {
    ...
    try {
      ...
      console.error('[dsh-docs-panel] Failed to read config (falling back to defaults): ', error && error.message ? error.message : String(error))
    }
    return DEFAULT_DIR
  }

  // 持久化配置到 config.json（惰性，仅在User主动Save时调用）
  async function persistConfiguredDir(dir) {
    const home = await getHome()
    if (!home) throw new Error('Could not get home directory')
    ...
    try {
      ...
      if (mkdirResult && mkdirResult.exitCode !== undefined && mkdirResult.exitCode !== 0) {
        throw new Error('mkdir failed, exitCode=' + mkdirResult.exitCode)
      }
    } catch (error) {
      throw new Error('Failed to create config directory: ' + (error && error.message ? error.message : String(error)))
    }
    const target = await fs.resolve(cfgPath)
  ...
  async function getHome() {
    ...
    try {
      ...
      console.error('[dsh-docs-panel] Failed to get home directory: ', error && error.message ? error.message : String(error))
    }
    return homeCache
  }
  ...
  // 校验 rel 并返回Documents directory目标与Files目标（含越界防护）
  async function resolveDocTargets(rel) {
    if (!isSafeRel(rel)) return { ok: false, error: 'Invalid filename' }
    ...
    try {
      ...
      return { ok: false, error: 'File inaccessible' }
    }
    if (!fs.contains(dirTarget, fileTarget)) return { ok: false, error: 'Files outside the documents directory cannot be accessed' }
  ...
  // SaveDocuments directory配置（校验目录存在后写入 config.json）
  async function handleConfigSet(args) {
    const dir = args && typeof args.docsDir === 'string' ? args.docsDir.trim() : ''
    if (dir.length === 0) return { ok: false, error: 'Path cannot be empty' }
    ...
    try {
      ...
      return { ok: false, error: 'Path inaccessible: ' + dir }
    }
    if (info === undefined) return { ok: false, error: 'Directory does not exist: ' + dir }
    if (info.type !== 'directory') return { ok: false, error: 'Not a folder: ' + dir }
    try {
      await persistConfiguredDir(dir)
      return { ok: true, docsDir: dir }
    } catch (error) {
      return { ok: false, error: 'Save failed: ' + (error && error.message ? error.message : String(error)) }
    }
  }

  // 递归列出Documents directory下的所有 Markdown Files
  async function handleDocsList() {
    ...
    try {
      ...
      return { ok: false, error: 'Directory inaccessible: ' + rawDir }
    }
    let rootInfo
    try {
    ...
    if (rootInfo === undefined) return { ok: false, error: 'Directory does not exist: ' + rawDir + '(changeable in panel settings)' }
    if (rootInfo.type !== 'directory') return { ok: false, error: 'Not a folder: ' + rawDir }
  ...
  async function handleDocsRead(args) {
    ...
    if (info === undefined) return { ok: false, error: 'File does not exist' }
    if (info.type !== 'file') return { ok: false, error: 'Not a regular file' }
    if (typeof info.size === 'number' && info.size > MAX_DOC_BYTES) return { ok: false, error: 'File too large; opening not supported yet' }
    try {
      const text = await fs.readText(resolved.fileTarget)
      return { ok: true, content: text, rel: rel }
    } catch (error) {
      return { ok: false, error: 'Read failed: ' + (error && error.message ? error.message : String(error)) }
    }
  }
  ...
  async function handleDocsOpen(args) {
    ...
    if (appName === undefined) return { ok: false, error: 'Unsupported app' }
    const resolved = await resolveDocTargets(rel)
    if (!resolved.ok) return { ok: false, error: resolved.error }
    ...
    try {
      ...
      return { ok: false, error: 'Open failed: ' + (errText || 'App may not be installed') }
    } catch (error) {
      return { ok: false, error: 'Open failed: ' + (error && error.message ? error.message : String(error)) }
    }
  }

  // CopyText到System剪贴板（macOS pbcopy）
  async function handleDocsCopy(args) {
    const text = args && typeof args.text === 'string' ? args.text : ''
    if (text.length === 0) return { ok: false, error: 'Nothing to copy' }
    if (text.length > MAX_COPY_BYTES) return { ok: false, error: 'Content too long to copy' }
    try {
      ...
      return { ok: false, error: 'Copy failed: ' + (error && error.message ? error.message : String(error)) }
    }
  }
  ...
  // 客户端 RPC：HTTP 路由（正式Plugins形态，重启后依然生效）
  if (webServer !== undefined) {
    ...
    console.log('[dsh-docs-panel] /docs-panel/api route registered')
  } else {
    console.warn('[dsh-docs-panel] webServer service unavailable; client interface not registered')
  }
}
