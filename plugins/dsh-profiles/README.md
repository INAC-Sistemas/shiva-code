...
A different profile always lands in a **new conversation**: an open session keeps the agent composition it started with. With a restart, the gate clears the open-session selection (`ctx.sessions.clear()`, which forgets `dsh.sessions.current`) before restarting, so the reloaded page opens a blank session in the most recent workspace; without one, it opens a new session at once through `startSession()` on `ctx.uiWorkspace` (packaged harness) or `ctx.workspaces` (this repository). A `revision` refresh of the same profile keeps the conversation. The restart reuses the harness port, so the origin and the login session survive it.
